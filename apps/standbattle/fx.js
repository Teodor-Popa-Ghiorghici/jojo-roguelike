...
const DRAW = {
  ...
  burstBg(g, e, k, W, H) {
    g.save();
    g.globalAlpha = Math.max(0, 1 - k) * 0.4;
    for (let i = 0; i < 22; i++) {
      const a = (i / 22) * Math.PI * 2 + e.t;
      const r0 = 30 + k * 120;
      poly(g, [
        [e.x + Math.cos(a) * r0, e.y + Math.sin(a) * r0],
        [e.x + Math.cos(a + 0.06) * (r0 + 200), e.y + Math.sin(a + 0.06) * (r0 + 200)],
        [e.x + Math.cos(a - 0.06) * (r0 + 200), e.y + Math.sin(a - 0.06) * (r0 + 200)]
      ], e.color || '#FFFFFF');
    }
    g.restore();
...
export function wireFx(combat, fx, groundY) {
  ...
  d.on('onHit', ev => {
    ...
    if (ev.combo === 3 || ev.combo === 6 || ev.combo >= 9) {
      ...
      fx.spawn('bark', { text: BARKS[i], x: P.x + P.facing * 30, y: groundY - 96, scale: 2 + i, life: 0.55 });
    }
  });

  d.on('onParrySuccess', () => {
    const x = P.x + P.facing * 20, y = groundY - 56;
    fx.spawn('flash', { color: '#FFFFFF', alpha: 0.55, life: 0.16 });
    fx.spawn('burstBg', { x, y, life: 0.4, color: FX.guard[4] });
    fx.spawn('shock', { x, y, size: 66, life: 0.4, color: FX.guard[4] });
    fx.spawn('impact', { x, y, size: 16, dir: 0, life: 0.35, big: true });
    fx.spawn('bark', { text: 'PARRY', x, y: groundY - 100, scale: 3, life: 0.7, color: FX.guard[4], shadow: '#0B2E4A' });
  });

  d.on('onDodgeSuccess', () => {
    fx.spawn('dust', { x: P.x, y: groundY - 2, dir: -P.facing, size: 20, life: 0.4 });
    fx.spawn('bark', { text: 'MISS', x: P.x, y: groundY - 94, scale: 2, life: 0.5, color: FX.ghost[4], shadow: '#152A55' });
  });
  ...
  d.on('onKill', () => {
    ...
    fx.spawn('flash', { color: '#FFFFFF', alpha: 0.45, life: 0.2 });
    fx.spawn('burstBg', { x, y, life: 0.55, color: FX.spark[3] });
  ...
  d.on('onPhaseTransition', () => {
    fx.spawn('flash', { color: '#FF4A7E', alpha: 0.5, life: 0.35 });
    fx.spawn('burstBg', { x: E.x, y: groundY - 60, life: 0.7, color: '#FF4A7E' });
