...
function pixelBMP(w, h, paint) {
  let rects = '';
  const px = (x, y, ww, hh, c) => {
    rects += '<rect x="' + x + '" y="' + y + '" width="' + ww + '" height="' + hh + '" fill="' + c + '"/>';
  };
  paint(px);
  const svg =
    '<svg xmlns="http://www.w3.org/2000/svg" width="' + w + '" height="' + h + '" ' +
    'viewBox="0 0 ' + w + ' ' + h + '" shape-rendering="crispEdges">' +
    '<rect width="' + w + '" height="' + h + '" fill="#000000"/>' + rects + '</svg>';
  return 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg);
}
const IMG_TEMPLE = pixelBMP(64, 48, px => {
  [[4,4],[12,9],[22,3],[38,6],[52,4],[58,12],[30,10],[46,15],[8,18],[60,22]]
    .forEach(p => px(p[0], p[1], 1, 1, '#FFFFFF'));
  px(31, 6, 2, 5, '#FFFF55');
  px(29, 7, 6, 2, '#FFFF55');
  for (let i = 0; i < 5; i++) px(28 - i*5, 12 + i*2, 8 + i*10, 2, '#FFFF55');
  px(6, 22, 52, 3, '#FFFFFF');
  px(24, 25, 16, 14, '#00AAAA');
  [10, 20, 30, 40, 50].forEach(x => {
    px(x, 25, 5, 14, '#AAAAAA');
    px(x, 25, 1, 14, '#FFFFFF');
    px(x + 4, 25, 1, 14, '#555555');
  });
  px(4, 39, 56, 3, '#AAAAAA');
  px(2, 42, 60, 3, '#555555');
  px(0, 45, 64, 3, '#AAAAAA');
});
const IMG_GLYPH = pixelBMP(16, 16, px => {
  px(0, 0, 16, 16, '#0000AA');
  px(1, 1, 14, 14, '#000000');
  px(7, 2, 2, 12, '#FFFF55');
  px(4, 5, 8, 2, '#FFFF55');
  px(6, 1, 4, 1, '#FFFFFF');
  px(3, 4, 1, 1, '#55FFFF'); px(12, 4, 1, 1, '#55FFFF');
  px(3, 8, 1, 1, '#55FFFF'); px(12, 8, 1, 1, '#55FFFF');
  px(2, 13, 12, 1, '#555555');
});
...
function stampPaths(node, prefix) {
  node.path = prefix;
  if (node.children) {
    node.children.forEach(c => stampPaths(c, prefix + '/' + c.name));
  }
}
stampPaths(FS, '::');
const seed = [];
