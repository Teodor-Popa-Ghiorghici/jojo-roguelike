...
`docs/qa/replays/` (README covers a `tailFrames` caveat).
...
- QA-005 (S2, new) — Long-Range retreat AI has no z-awareness; enemies
  never move in z after spawn; parks the User at a z-extreme melee can't
  reach. Reproduced via the shipped Pinned encounter (`shopping_street_pinned`):
  zero damage for its whole 2400f window.
- QA-006 (S2, new) — Mid-Range's flicked Stand has no per-frame clamp;
  Mid never roots the User, so held movement outraces Strain drag,
  growing the tether well past Close's 1.4x-equivalent. Long-Range shows
  the same gap in its base case. Same seam as QA-001.
- QA-007 (S2, new) — z-parking (any scheme) causes a mutual melee whiff
  stalemate; not a true softlock (self-resolved by moving back) — 13l
  should judge if it's worth acting on.
- #1/#2/#4 clean for Close-Range at all 4 corners. #3 moot (no collision
  system, per `fighter.js`). #6: Cairo/Naples "moving floor"/"terraces"
  aren't implemented as position mechanics (`hazardSet` unwired); only
  the static damage-zone exists. #8: self-capping via life/x-OOB removal.
